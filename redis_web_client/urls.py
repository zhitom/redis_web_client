...
urlpatterns = [
	...
	url(r'^view/(?P<value_redis_id>[0-9]+)/(?P<value_db_id>[0-9]+)/(?P<key>.*)/', GetValueView.as_view(), name='getvalue'),
	url(r'^client/redis(?P<client_id>[0-9]+)', ClientHtmlView.as_view(), name='client_html'),
	url(r'^client_list/', ClientListView.as_view(), name='client_list'),
	url(r'^del/key/', DelKeyView.as_view(), name='del_key'),
	url(r'^edit/redis(?P<edit_server_id>[0-9]+)/db(?P<edit_db_id>[0-9]+)', EditValueTableView.as_view(), name='edit_key'),
	url(r'^bgsave/redis(?P<bg_server_id>[0-9]+)/', BgSaveView.as_view(), name='bg_save'),
