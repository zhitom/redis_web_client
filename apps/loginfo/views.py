	...
	def get(self, request):
		...
		if request.GET.get('type', None) == 'json':
			...
			if search_data is not None:
				...
				db_data = list(OperationInfo.objects.filter(Q(type='edit') & Q(key__contains=search_data))[min_num:max_num].values())
			else:
	...
	def get(self, request):
		...
		if request.GET.get('type', None) == 'json':
			...
			if search_data is not None:
				...
				db_data = list(OperationInfo.objects.filter(Q(type='del') & Q(key__contains=search_data))[min_num:max_num].values())
			else:
